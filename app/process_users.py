...
def clean_list(lst):
    ...
    cleaned_list = [difflib.get_close_matches(item.lower(), case_name, n=1, cutoff=0.465)[0]
                    for item in lst if difflib.get_close_matches(item.lower(), case_name, n=1, cutoff=0.445)]
    return cleaned_list
